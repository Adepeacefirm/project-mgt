...
import { addProject, setMembers } from "../features/workspaceSlice";
...
const CreateProjectDialog = ({ isDialogOpen, setIsDialogOpen }) => {
  ...
  useEffect(() => {
    if (!memberships?.data || !currentWorkspace) return;

    const formattedMembers = memberships.data.map((m) => ({
      id: m.id,
      user: {
    ...
    // GUARD: Compare existing members with new ones to stop the loop
    const existingEmails = currentWorkspace.members
      ?.map((m) => m.user.email)
      .join(",");
    const newEmails = formattedMembers.map((m) => m.user.email).join(",");

    if (existingEmails !== newEmails) {
      dispatch(setMembers(formattedMembers));
    }
  }, [memberships?.data, dispatch, currentWorkspace]);
